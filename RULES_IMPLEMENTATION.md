...
| **2a. Map & Exploration (A/B/C, Exploration deck)** | **Not Implemented** | Engine uses a **static 5-room test map**; there is **no Exploration deck**, no on-the-fly Corridor/Room setup, no Secure or Reinforced Corridors, and no Technical Corridors. UI shows a fixed circular layout. |
...
| **5. Combat (Shooting / Melee)** | **Partial** | **Divergence vs official rules**:<br/>• *Official:* Shooting **does not** spend ammo except on the “last-bullet” die result; Burst action spends ammo & uses a Burst die.<br/>• *Current:* `SHOOT` **always** consumes 1 ammo; **no `BURST` action yet**; attack outcomes via deterministic table (`_attack_outcome`).<br/>Other details unchanged: `_attack_outcome` returns `hit/-1 HP`, `crit/-2 HP`, `jam`, `miss`; MELEE inflicts mutual damage; state tracks `weapon_jammed`, `serious_wounds`.<br/>Tests: `test_actions_doors_shoot_room.py`, `test_ammo_and_attack_deck.py`. |
